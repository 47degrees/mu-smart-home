...
object SmartHomeSupervisor {
  implicit def instance[F[_]: Sync: Logger](
      implicit topicPubSubClient: Resource[F, PubsubProducer[F, Row]]
  ): SmartHomeSupervisor[F] = new SmartHomeSupervisor[F] {
    ...
    override def performAction(location: Location): F[List[SmartHomeAction]] =
      publishLocation(location) *> Sync[F].delay(
    ...
    private def publishLocation(location: Location): F[MessageId] = {
      topicPubSubClient.use { producer =>
        producer.produce(
          Row(
            Instant.now,
            location.currentLocation.map(_.lat).getOrElse(0d),
            location.currentLocation.map(_.long).getOrElse(0d)
          )
        )
      }
    }
